...
use Mugenzo\LaravelShoppingCart\Contracts\Buyable;
use Mugenzo\LaravelShoppingCart\Contracts\InstanceIdentifier;
use Mugenzo\LaravelShoppingCart\Exceptions\CartAlreadyStoredException;
use Mugenzo\LaravelShoppingCart\Exceptions\InvalidRowIDException;
use Mugenzo\LaravelShoppingCart\Exceptions\UnknownModelException;
...
class Cart
{
     ...
     */
    public function __construct(SessionManager $session, Dispatcher $events, Discount $discount)
    {
    ...
    public function instance($instance = null)
    {
        ...
        if ($instance instanceof InstanceIdentifier) {
            ...
            $this->discountValue = $instance->getInstanceGlobalDiscountValue();
    ...
    {
        $discount         = $this->discount->retrieveDiscount();
        $initialCartPrice = $this->getContent()->reduce(function ($price, CartItem $cartItem) {
            return $price + $cartItem->total;
        },
            0);

        if ($discount['type'] === 'percent') {
            return $initialCartPrice * ($discount['value'] / 100);
        }
    ...
    {
        return $this->initialFloat() - $this->discountFloat();
    }
     ...
     * @return void
     */
    public function setDiscountValue($rowId, $discountValue, $discountType = 'percent')
    {
        $cartItem = $this->get($rowId);

        $cartItem->setDiscountValue($discountValue, $discountType);
     ...
     * @return void
     */
    public function setGlobalDiscount(float $discountValue, $discountType = 'percent')
    {
        $this->discount->setDiscount($discountType, $discountValue);
    }
    ...
    {
        return config('shopping_cart_config.database.table', 'shoppingcart');
    }
    ...
    {
        $connection = config('shopping_cart_config.database.connection');
    ...
    {
        if (is_null($decimals)) {
            $decimals = config('shopping_cart_config.format.decimals', 2);
        }

        if (is_null($decimalPoint)) {
            $decimalPoint = config('shopping_cart_config.format.decimal_point', '.');
        }

        if (is_null($thousandSeparator)) {
            $thousandSeparator = config('shopping_cart_config.format.thousand_separator', ',');
        }
